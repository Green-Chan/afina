...
class Argv {
public:
    Argv(std::initializer_list<const char *> args) : m_argv(new char *[args.size()]), m_argc(args.size()) {
        int i = 0;
...
TEST_CASE("Basic options", "[options]") {
    ...
    options.add_options()("long", "a long option")("s,short", "a short option")("value", "an option with a value",
                                                                                cxxopts::value<std::string>())(
        "a,av", "a short option with a value", cxxopts::value<std::string>())("6,six", "a short number option")(
...
TEST_CASE("Short options", "[options]") {
    ...
    REQUIRE_THROWS_AS(options.add_options()("", "nothing option"), cxxopts::invalid_option_format_error);
}

TEST_CASE("Required arguments", "[options]") {
    cxxopts::Options options("required", " - test required options");
    options.add_options()("one", "one option")("two", "second option");

    Argv argv({"required", "--one"});

    auto aargv = argv.argv();
    auto argc = argv.argc();

    options.parse(argc, aargv);
    REQUIRE_THROWS_AS(cxxopts::check_required(options, {"two"}), cxxopts::option_required_exception);
}

TEST_CASE("No positional", "[positional]") {
    cxxopts::Options options("test_no_positional", " - test no positional options");
...
TEST_CASE("All positional", "[positional]") {
    ...
    options.add_options()("positional", "Positional parameters", cxxopts::value<std::vector<std::string>>(positional));
...
TEST_CASE("Some positional explicit", "[positional]") {
    ...
    options.add_options()("input", "Input file", cxxopts::value<std::string>())(
        "output", "Output file", cxxopts::value<std::string>())("positional", "Positional parameters",
...
TEST_CASE("Empty with implicit value", "[implicit]") {
    ...
    options.add_options()("implicit", "Has implicit", cxxopts::value<std::string>()->implicit_value("foo"));
...
TEST_CASE("Integers", "[options]") {
    ...
    options.add_options()("positional", "Integers", cxxopts::value<std::vector<int>>());
...
TEST_CASE("Unsigned integers", "[options]") {
    ...
    options.add_options()("positional", "Integers", cxxopts::value<std::vector<unsigned int>>());
...
TEST_CASE("Integer bounds", "[integer]") {
    ...
    options.add_options()("positional", "Integers", cxxopts::value<std::vector<int8_t>>());
...
TEST_CASE("Floats", "[options]") {
    ...
    options.add_options()("double", "Double precision", cxxopts::value<double>())("positional", "Floats",
